...
from app.utility.special.payload_encoder import xor_file
...
class FileSvc(BaseService):
    ...
    async def download(self, request):
        ...
        try:
            ...
            if payload in self.special_payloads:
                ...
                content = self.data_svc.get_payload(payload)
    ...
    async def payload_download(self, request):
        """
        Accept a request with a required header, file, and an optional header, platform, and download the file in mongodb.
        :param request:
        :return: a multipart file via HTTP
        """
        try:
    ...
    async def read_file(self, name):
        """
        find a file in mongodb and read the contents
        :param name:
    ...
    async def compile_go(platform, output, src_fle, ldflags='-s -w'):
        ...
        :return:
        """
        os.system('GOOS=%s /usr/local/go/bin/go build -o %s -ldflags="%s" %s' % (platform, output, ldflags, src_fle))
